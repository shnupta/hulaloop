...
public:
	...
	[[nodiscard]] closer schedule(callback cb)
	{
		return schedule(0ns, cb);
	}
	...
	uint64_t post(std::chrono::nanoseconds fire_in, callback cb)
	{
		...
		while (it != _callback_contexts.rend() && fire_at > it->_fire_at)
			it++;

		_callback_contexts.insert(it.base(), callback_context{
				._id = id,
				._fire_at = fire_at,
				._cb = cb
				});
	...
	{
		auto it = std::find_if(_callback_contexts.begin(), _callback_contexts.end(), [=](const callback_context& ctx)
				{
					return ctx._id == id;
				});
	...
	[[nodiscard]] closer connect_to_unix_signal(unix::sig s, unix::signal::slot sl)
	{
		unix::signal& sig = register_or_get_unix_signal(s);
		return sig.connect(sl);
	}

	// register a handler to the given fd (this will cause the loop to poll the file descriptor).
	// to stop polling the descriptor simply close the handle.
...
private:
	...
	void do_cycle()
	{
		...
		while (!_callback_contexts.empty() && _callback_contexts.back()._fire_at <= now && !_stopping)
		{
	...
	unix::signal& get_unix_signal(unix::sig s)
	{
		...
		{
			throw std::runtime_error("unexpected get for signal handler: " + std::to_string(static_cast<int>(s)));
		}
