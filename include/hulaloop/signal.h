 ...
 private:
  void disconnect(uint64_t id) {
    auto it = std::find_if(_slots.begin(), _slots.end(),
                           [=](const slot_info& s) { return s._id == id; });
    if (_during_call) {
      if (it != _slots.end()) {
        it->_active = false;
      }
    ...
    auto pending_it =
        std::find_if(_pending_connections.begin(), _pending_connections.end(),
                     [=](const slot_info& s) { return s._id == id; });
  ...
  void do_pending_disconnects() {
    assert(!_during_call);
    for (const auto id : _pending_disconnections) disconnect(id);
  ...
  struct slot_info {
    ...
    template <class U = T, typename = std::enable_if<!std::is_void_v<T>, void>>
    void operator()(U&& arg) {
      if (_active) _slot(std::forward<U>(arg));
    }
  };
